...
- 关键设计：买卖落地时会同步生成或关联 `accounts.Transaction` 作为现金流水；持仓按 `user + instrument` 唯一聚合；卖出必须生成已实现盈亏。
...
- Key design: buy/sell execution also creates or links `accounts.Transaction` cash entries; positions are unique per `user + instrument`; sell records must carry realized PnL.
