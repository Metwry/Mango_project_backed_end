...
from .core_indices import CORE_INDEX_DEFINITIONS, index_definition_by_symbol
from .cache import MARKET_INDEX_QUOTES_KEY, MARKET_INDEX_QUOTES_MARKET_KEY_PREFIX
from .pull_guard import market_guard_decision
        ...
        rows.append(
            {
                ...
                "instrument_id": inst.id,
                "name": inst.name,
                "prev_close": format_decimal_str(prev_close),
                "day_high": format_decimal_str(current * Decimal("1.002")),
                "day_low": format_decimal_str(prev_close * Decimal("0.998")),
                "pct": format_decimal_str(Decimal("0.5")),
            }
        )
    return rows
...
def pull_indices(*, now_local: datetime | None = None) -> dict:
    ...
    for market, instruments in grouped.items():
        ...
        decision = market_guard_decision(market, now_utc=current_time)
        should_pull = decision.should_pull
